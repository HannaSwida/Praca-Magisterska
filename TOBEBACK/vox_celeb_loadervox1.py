...
def load(path, num_samples=3200):
    wav, sr = librosa.load(path, sr=16000)
    start = random.randint(0, wav.shape[0] - num_samples)

    print(wav[start: (start + num_samples)].shape, "after shape")
    # print(wav.shape[0])
...
class VoxCelebLoader(Dataset):
    ...
    def __getitem__(self, _):
        ...
        print(speaker1, speaker2, "spks")  # names
        [speaker1utt1, speaker1utt2] = random.sample(self.speakers[speaker1], 2)
        print(speaker1utt1, "a", speaker1utt2)  # sciezki
